...
function postgresql_install_update() {
    local install_or_update="$1"
    local postgres_password="$1"

    if [ -z "$install_or_update" ]; then
        # Demander au développeur s'il souhaite installer ou mettre à jour PostgreSQL
    ...
    if [ -z "$postgres_password" ]; then
        # Définir le mot de passe pour l'utilisateur root PostgreSQL
        read -p "Entrez le mot de passe pour l'utilisateur root PostgreSQL : " postgres_password
    fi

    # Vérifier si le mot de passe est spécifié
    if [ -z "$postgres_password" ]; then
        echo "Veuillez spécifier un mot de passe pour l'utilisateur root PostgreSQL."
        exit 1
    fi
